...
/**
 * Lista todos os PIX do beneficiário logado
 */
...
const gerar = async (req, res) => {
  try {
    ...
    const dadosFaltantes = dadosObrigatorios.filter(
      (dadoObrigatorio) => !dados[dadoObrigatorio]
    );
...
const selecionar = async (req, res) => {
  try {
    ...
    const pixResponse = await abacatePayService.selecionarPix(
      pix.id_integracao
    );
...
const simularPagamento = async (req, res) => {
  try {
    ...
    const pixResponse = await abacatePayService.simularPagamentoPix(
      pix.id_integracao
    );
    ...
    return res
      .status(200)
      .json({ mensagem: "Pagamento simulado com sucesso", pix });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
export default { listar, gerar, selecionar, simularPagamento };
